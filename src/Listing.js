...
function Listing({ listingData }) {
  ...
  return (
    <li style={{ display: 'flex', alignItems: 'center' }}>
      <img
        src={listingData.image || defaultImageUrl}
        alt="Property"
        style={{ width: '50px', height: '50px', marginRight: '10px' }}
      />
      <div>
        <p style={{ fontSize: '24px', fontWeight: 'bold' }}>Address: {listingData.streetAddress}</p>
        <p>List Price: ${listingData.listPrice}</p>
        <p style={{ color: getEstimatePriceColor(listingData) }}>
          Estimated Price: ${listingData.estimatePrice}
        </p>
      </div>
    </li>
  );
...
function getEstimatePriceColor(listingData) {
  const difference = listingData.estimatePrice - listingData.listPrice;
